...
/**
 * Port de `Views/Nova/Agent/NovaToolRegistry.swift`.
 * 28 declarations (activity+social+profile+memory) + [confirmationRequiredTools].
 * Δ: iOS `toolSet` es `Tool.functionDeclarations(…)`; aquí la lista cruda —
 * el wrap a Firebase `Tool` (+ googleSearch) vive en [NovaAIService.makeChatModel].
 */
object NovaToolRegistry {
    ...
    /** ≡ iOS declarations alimentando `Tool.functionDeclarations`. */
    ...
    private val profileTools get() = listOf(
        ...
        declaration(
            ...
            "Return a combined neutral JSON overview of the current user's profile, recent moments, and recent stories.",
            mapOf(
                "moment_limit" to integer("Max moments to include (default 5, max 10)."),
                "story_limit" to integer("Max stories to include (default 5, max 10)."),
            ),
            setOf("moment_limit", "story_limit"),
        ),
        ...
        declaration(
            ...
            "Update the current user's privacy settings. Requires user confirmation. Only include keys the user actually wants to change.",
            mapOf(
                "is_private" to bool("Whether the account should be private."),
                "show_mutuals" to bool("Whether mutuals should be visible."),
                "show_following" to bool("Whether following should be visible."),
                "show_followers" to bool("Whether followers should be visible."),
            ),
            setOf("is_private", "show_mutuals", "show_following", "show_followers"),
        ),
        declaration(
            "update_profile_bio",
            "Update the current user's profile bio. Requires user confirmation. Use an empty string only if the user explicitly wants to clear it.",
            mapOf("bio" to string("New bio text.")),
        ),
        declaration(
            "update_profile_website",
            "Update the current user's profile website. Requires user confirmation. Use an empty string only if the user explicitly wants to clear it.",
            mapOf("website" to string("New website URL.")),
        ),
        declaration(
            "update_active_hours",
            "Update or clear the current user's active hours. Requires user confirmation.",
            mapOf(
                "start_hour" to string("Start hour in HH:mm format."),
                "end_hour" to string("End hour in HH:mm format."),
                "clear" to bool("Set true only when explicitly clearing active hours."),
            ),
            setOf("start_hour", "end_hour", "clear"),
        ),
        declaration(
            "update_notification_preferences",
            "Update one or more notification preferences for the current user. Requires user confirmation.",
            notificationParameters,
            notificationParameters.keys,
        ),
        declaration(
            "get_user_profile_snapshot",
            "Return a neutral JSON profile snapshot. Defaults to the current user if no identifier is supplied.",
            mapOf(
                "username" to string("Optional username, with or without @."),
                "user_id" to string("Optional target user id."),
            ),
            setOf("username", "user_id"),
        ),
        declaration(
            "get_moment_details",
            "Return neutral JSON details for a specific moment id if available to the current user.",
            mapOf("moment_id" to string("Concrete moment id.")),
        ),
    ...
    private fun limited(name: String, description: String, limitDescription: String) =
        declaration(name, description, mapOf("limit" to integer(limitDescription)), setOf("limit"))
    ...
    private val memoryTools get() = listOf(
        declaration(
            "remember_fact",
            "Persist a durable fact the user explicitly asked to remember. Requires user confirmation. Do NOT use for names (use update_user_preference) or info already in context.",
            mapOf(
                "content" to string("Fact in the user's language."),
                "type" to string("One of: preference, personal, professional, interest, general."),
            ),
            setOf("type"),
        ),
        declaration(
            "update_user_preference",
            "Update a user preference. Requires user confirmation. Use key preferred_name for names.",
            mapOf(
                "key" to string("Preference key, e.g. preferred_name or tone."),
                "value" to string("Preference value."),
            ),
        ),
    )
