...
/**
 * Helpers globales portados de NotificationExtensions.swift (NotificationCenter / Kingfisher).
 * SharedFlow ≡ Notification.Name; Coil ≡ Kingfisher cache.
 */
object AppEventBus {
    ...
    private val _conversationUpdated = MutableSharedFlow<Unit>(extraBufferCapacity = 8)
    val conversationUpdated: SharedFlow<Unit> = _conversationUpdated.asSharedFlow()

    private val _commentAdded = MutableSharedFlow<Unit>(extraBufferCapacity = 8)
    val commentAdded: SharedFlow<Unit> = _commentAdded.asSharedFlow()
    ...
    fun postConversationUpdated() {
        _conversationUpdated.tryEmit(Unit)
    }

    fun postCommentAdded() {
...
/** ≡ ImageCacheManager (NotificationExtensions.swift). */
object ImageCacheManager {
    fun clearCacheForUser(userId: String) = Unit

    fun clearAllCache() {
        val ctx = MomentsApplication.instance ?: return
